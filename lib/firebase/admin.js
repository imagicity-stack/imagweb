...
// Creacity ("Tips & Tricks") lives in a SEPARATE Firestore database in the same
// Firebase project, using the same credentials/auth/storage. Only the database
// id differs — set CREACITY_DATABASE_ID in Vercel.
export const creacityDatabaseId = process.env.CREACITY_DATABASE_ID || "";
export const isCreacityConfigured = isAdminConfigured && Boolean(creacityDatabaseId);
...
// Firestore handle for the Creacity database.
export function getCreacityDb() {
  const app = getAdminApp();
  return getFirestore(app, creacityDatabaseId);
}
